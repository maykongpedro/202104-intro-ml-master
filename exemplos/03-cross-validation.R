...
# aqui estamos tunando a complexidade, e não a altura da árvore
...
diamonds_tune_grid <- tune_grid(
  ...
  # com verbose igual a TRUE conseguimos saber onde o modelo está processando
)
...
diamonds_last_fit <- diamonds_model %>% last_fit(price ~ x, 
                                                 split = diamonds_initial_split)
